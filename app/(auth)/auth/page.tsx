...
const AuthPage = () => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-red-50/30 via-orange-50/20 
          to-yellow-50/30 dark:from-red-950/10 dark:via-orange-950/10
           dark:to-yellow-950/10">
            {/*Background pattern */}
            <div className='absolute inset-0 bg-[url("data:image/svg+xml,%3Csvg width="60" 
                ...
                fill="none" fill-rule="evenodd"%3E%3Cg fill="%23f59e0b" 
                fill-opacity="0.05"%3E%3Ccircle cx="30" cy="30" 
                r="4"/%3E%3C/g%3E%3C/g%3E%3C/svg%3E")] opacity-30' >
                <div className="relative w-full max-w-md sm:max-w-lg md:max-w-2xl lg:max-w-4xl
                       mx-auto flex flex-col justify-center min-h-screen p-3 xs:p-4 sm:p-6 lg:p-8">
                    <div className="bg-white/80 dark:bg-gray-900/80 backdrop-blur-sm 
                          rounded-2xl xs:rounded-3xl shadow-2xl border border-orange-200/50
                           dark:border-orange-800/50 p-4 xs:p-6 sm:p-8 lg:p-12">
                            {/* Back Button */}
                          <div className="mb-4 xs:mb-5 sm:mb-6 lg:mb-8 cursor-pointer inline-flex 
                                items-center justify-center w-9 h-9 xs:w-10 xs:h-10 sm:w-12 sm:h-12 
                                rounded-xl xs:rounded-2xl bg-orange-100 dark:bg-orange-900/30 
                                border border-orange-200 dark:border-orange-800 
                                hover:bg-orange-200 dark:hover:bg-orange-900/50 transition-all 
                                duration-200 hover:scale-105 active:scale-[0.95]">
                                <ChevronLeft className="text-orange-600 dark:text-orange-400
                                      h-4 w-4 xs:h-5 xs:w-5 sm:h-6 sm:w-6"/>
                          </div>
                          {/* Form header */}
                          <div className="text-center mb-6 xs:mb-7 sm:mb-8 lg:mb-10">
                            <div className="inline-flex items-center px-3 xs:px-4 py-1.5 
                                xs:py-2 rounded-full bg-orange-100 dark:bg-orange-900/30 
                                border border-orange-200 dark:border-orange-800 mb-4 xs:mb-5 
                                sm:mb-6">
                              <span className="text-xs xs:text-sm font-semibold text-orange-800 dark:text-orange-200">
                                {isSignUp ? 'Join PrimaSlice Pizza' : 'Welcome Back'}
                              </span>
                            </div>
                          ...
                          <p className='text-sm xs:text-base sm:text-lg md:text-xl lg:text-2xl text-gray-600 dark:text-gray-300 leading-relaxed max-w-2xl mx-auto px-2'>
                            {isSignUp
                              ? '🍕 Join Slice of Prima today and discover exclusive deals on authentic Italian cuisine delivered hot to your door!'
                              : '🎉 Welcome back to Slice of Prima! Log in to continue your delicious journey with us.'}
                          </p>
                          </div>
